...
```json
{
    "title": "生为女人",
    "author": "川端康成",
    "description": "日本新感觉主义文学巅峰之作",
    "isbn": "9787536692179",
    "publishYear": 1958,
    "coverUrl": "https://img3.doubanio.com/view/subject/s/public/s3078482.jpg",
    "total": 15,
    "tagIds": [3]
}
```
...
### 获取借阅图书次数最多的前`limit`名用户

- 接口路径：`/statistics/top-users`
- 请求方法：`GET`
- 接口描述：获取借阅数量最多的用户排行榜
...
### 获取被用户借阅次数最多的前`limit`本图书

- 接口路径：`/statistics/top-books`
- 请求方法：`GET`
- 接口描述：获取被借阅次数最多的图书排行榜

#### 请求参数

| 字段名 | 类型 | 必传 | 默认值 | 描述               | 其他     |
| ------ | ---- | ---- | ------ | ------------------ | -------- |
| limit  | int  | 否   | 5      | 限制返回的图书数量 | 查询参数 |
...
### 获取动态（最新`limit`条的借阅/归还记录）

- 接口路径：/statistics/activities
- 请求方法：GET
- 接口描述：获取最近的图书借阅动态
...
| 字段名 | 类型 | 必传 | 默认值 | 描述                             | 其他     |
| ------ | ---- | ---- | ------ | -------------------------------- | -------- |
| limit  | int  | 否   | 10     | 限制返回的动态数量，最大值为 100 | 查询参数 |
    ...
    "data": [
        {
            ...
            "bookTitle": "三体 (精装版)",
            "action": "归还",
            "time": "2025-12-04T17:31:57"
        },
        {
            "userName": "吕建超",
            "bookTitle": "三体 (精装版)",
            "action": "借阅",
            "time": "2025-12-04T17:30:30"
        }
    ],
    "success": true
}
